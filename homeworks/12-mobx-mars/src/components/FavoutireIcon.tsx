...
export const FavouriteIcon = observer(({photoId}: FavouriteIconProps) => {
  const className = "favicon";
  const favourites = useStore("Favourites");

  const isFavourite = favourites.favouritePhotos[photoId] ?? false;

  const opacity = isFavourite ? "1" : "0.7";

  const onFavouriteClick = (photoId: number): void => {
    favourites.favouriteStatusChange(photoId);
  };
