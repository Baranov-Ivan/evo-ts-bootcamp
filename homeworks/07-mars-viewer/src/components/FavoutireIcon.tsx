...
} from "../modules/storeSlices/favouritesSlice";
...
export function FavouriteIcon({ photoId }: FavouriteIconProps): JSX.Element {
  const className = "favicon";
  const isFavorite = useAppSelector(
    (state) => state.favourites.favouritePhotos[photoId] ?? false
  );
  const dispatch = useAppDispatch();
  const opacity = isFavorite ? "1" : "0.7";

  const onFavouriteClick = (photoId: number): void => {
    dispatch(favouriteStatusChange(photoId));
  };

  return (
    <svg
      className={className}
      onClick={() => onFavouriteClick(photoId)}
      width="98"
      height="89"
      viewBox="0 0 98 89"
      fill="none"
